...
export default function ContactScreen({ navigation, onClose }) {
  ...
  const whatsappWithDetails = () => {
    const lines = [
      `New callback request from PropEase:`,
      `Name: ${form.name}`,
      `Mobile: ${form.mobile}`,
      form.message.trim() ? `Message: ${form.message.trim()}` : null,
    ].filter(Boolean).join('\n');
    Linking.openURL(`https://wa.me/91${SUPPORT_PHONE}?text=${encodeURIComponent(lines)}`);
  };
  ...
  if (submitted) {
    return (
      <View style={styles.successContainer}>
        ...
        <Text style={styles.successDesc}>One tap sends your details straight to us on WhatsApp for the fastest response.</Text>
        <TouchableOpacity style={styles.callNowBtn} onPress={whatsappWithDetails}>
          <Text style={styles.callNowText}>💬 Send Details on WhatsApp</Text>
        </TouchableOpacity>

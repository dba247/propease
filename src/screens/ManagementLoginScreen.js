...
import { findAccount, registerAccount } from '../data/accountStore';
...
function ManagementSignupScreen({ navigation, onBack }) {
  ...
  const handleCreate = async () => {
    ...
    if (selectedCountry.code !== ALLOWED_COUNTRY || form.mobile.trim() !== ALLOWED_PHONE) {
      Alert.alert('Unauthorised', 'Management accounts must use the authorised phone number.');
      return;
    }
    if (form.email.trim().toLowerCase() !== ALLOWED_EMAIL) {

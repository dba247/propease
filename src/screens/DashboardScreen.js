...
import { getCustomers, getRentRecords, getMaintenanceRequests, getWorkOrders, getContactRequests, markContactRequestContacted } from '../data/propertyStore';
...
export default function DashboardScreen({ navigation }) {
  ...
  const loadContactRequests = () => {
    getContactRequests()
      ...
      .catch(e => console.error('[DashboardScreen] getContactRequests failed:', e));
  };
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.actionsGrid}>
          {[
            ...
            { label: 'Tenants',     icon: '🏘️', screen: 'Tenants' },
      ...
      {contactRequests.filter(r => r.status !== 'contacted').length > 0 && (
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>💬 Callback Requests</Text>
          {contactRequests
            .filter(r => r.status !== 'contacted')
            .map(r => (
              <View style={styles.requestCard} key={r.id}>
                <View style={{ flex: 1 }}>
                  <Text style={styles.requestName}>{r.name}</Text>
                  <Text style={styles.requestMobile}>📞 {r.mobile}</Text>
                  {r.message ? <Text style={styles.requestMessage}>{r.message}</Text> : null}
                  <Text style={styles.requestDate}>{r.submittedOn}</Text>
                </View>
                <TouchableOpacity
                  style={styles.contactedBtn}
                  onPress={() => {
                    markContactRequestContacted(r.id)
                      .then(loadContactRequests)
                      .catch(e => console.error('[DashboardScreen] markContactRequestContacted failed:', e));
                  }}
                >
                  <Text style={styles.contactedBtnText}>Mark Contacted</Text>
                </TouchableOpacity>
              </View>
            ))}
        </View>
...
const styles = StyleSheet.create({
  ...
  requestCard: {
    ...
    borderRadius: theme.radius.md, padding: 14, marginBottom: 10, ...theme.shadow,
  },
  requestName: { fontSize: 14, fontWeight: '800', color: theme.colors.text },
  requestMobile: { fontSize: 13, color: theme.colors.textSecondary, marginTop: 2 },
  requestMessage: { fontSize: 12, color: theme.colors.textLight, marginTop: 4, fontStyle: 'italic' },
  requestDate: { fontSize: 11, color: theme.colors.textLight, marginTop: 4 },
  contactedBtn: { backgroundColor: theme.colors.bg, borderRadius: 8, paddingVertical: 8, paddingHorizontal: 10, borderWidth: 1, borderColor: theme.colors.border },
  contactedBtnText: { fontSize: 11, fontWeight: '700', color: theme.colors.primary },
